...
// // ── Template preview (mobile bottom sheet) ─────────────────────────────────
// // Lets the user see a full-size render of a template (with their own data)
// // before committing to it, without disturbing the existing one-tap-select
// // flow on the grid. Mobile-only entry point — see the Eye button on each card.
...
// }) {
//   const data = form?.name !== "" ? form : DefaultData;
//   const PreviewLayout = (tmpl?.layout && layoutMap?.[tmpl?.layout]);

//   return (
//     <Sheet open={open} onOpenChange={onOpenChange}>
//       <SheetContent side="bottom" className="h-[88vh] flex flex-col p-0 gap-0">
//         <SheetHeader className="px-4 pt-4 pb-3 border-b border-gray-100 text-left">
//           <SheetTitle className="text-sm font-semibold">
//             {tmpl?.name ?? "Template preview"}
//           </SheetTitle>
//         </SheetHeader>

//         <div className="flex-1 overflow-auto bg-[#f7f4f0] flex items-start justify-center p-2.5">
//           {PreviewLayout ? (
//             <div className="bg-white shadow-lg border border-gray-200 overflow-hidden w-full max-w-full">
//               <ResponsivePageScaler>
//                 <div style={{ background: "#fff" }}>
//                   <PagedResumeWrapper>
//                     <PreviewLayout d={data} tk={tmpl?.tokens} />
//                   </PagedResumeWrapper>
//                 </div>
//               </ResponsivePageScaler>
//             </div>
...
//               <MiniResumeThumbnail tmpl={tmpl} scale={0.45} />
//             </div>
//           )}
//         </div>

//         <SheetFooter className="px-4 py-3 border-t border-gray-100 flex-row gap-2">
//           <SheetClose asChild>
...
//           >
//             {locked ? "⭐ Premium" : isSelected ? "Currently selected" : "Use this template"}
//           </Button>
//         </SheetFooter>
//       </SheetContent>
//     </Sheet>
...
//                   setPreviewTmpl(tmpl);
//                 }}
//                 className="sm:hidden absolute top-2 right-2 z-20 w-6 h-6 rounded-full bg-white/90
...
//         open={!!previewTmpl}
//         onOpenChange={(next) => {
//           if (!next) setPreviewTmpl(null);
//         }}
//         form={form}
//         DefaultData={DefaultData}
//         layoutMap={layoutMap}
//         isSelected={!!previewTmpl && selectedTmpl === previewTmpl.id}
//         locked={!!previewTmpl?.is_premium && user?.plan !== "premium"}
//         onUseTemplate={() => {
//           if (!previewTmpl) return;
//           handleSelect(previewTmpl.id, previewTmpl.is_premium);
//           setPreviewTmpl(null);
//         }}
//       />
//     </div>
...
//           <Markdown></Markdown>{generate.data.insight}
//         </div>
//       )}
//       {generate.isError && (
...
// Mirrors the project's `md` breakpoint (768px) so mobile and desktop never
// both render their respective tab UI at once.
...
export default function ResumeRightPanel({
  ...
  const handleTemplateSelect = useCallback(
    (id: string) => {
      ...
      if (found) setLocalActiveTmpl(found);
      setActiveTab("preview");
      // Switching templates from inside the mobile sheet should drop back to
      // the resume itself rather than leaving the Templates sheet open.
